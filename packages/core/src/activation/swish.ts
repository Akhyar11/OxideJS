...
import { isNativeAvailable, swishNative } from "../math/rust_backend.js";

export default function swish(a: Matrix): Matrix {
  const result = mj.zeros([...a._shape]);
  const dResult = mj.zeros([...a._shape]);

  if (isNativeAvailable()) {
    swishNative(a._data, result._data, dResult._data);
  } else {
